...
\begin{figure}[H]
  \centering
    \includegraphics[width=0.7\textwidth]{img/Diagrama2.eps}
  \caption{Diagrama de bloques del robot.}
\end{figure}
\newpage

Para llevar a cabo este proyecto se fabricó un robot. El mismo esta compuesto de las siguientes partes:
...
Para tener la libertad de poder cambiar la velocidad de los motores, se utilizó una señal PWM. Se configuró el \emph{timer/contador1} en modo \emph{Fast PWM}. De esta forma, el contador se incrementa de $0$ a $256$ cuentas. Cuando alcanza este último valor, desborda y comienza otra vez desde cero. Luego, realizando dos comparaciones, se puede variar el ancho de pulso de dos señales que se usan para regular la potencia entregada a cada motor.

También se utilizaron dos puentes H \emph{LMD18200} para poder frenar y cambiar el sentido de giro de cada motor por separado.\\

Los motores del robot son de la marca \emph{Pololu}, los mismos vienen equipados con reducciones $29:1$ y encoders en cuadratura con una resolución de 64 pasos por vuelta de motor.
Gracias a la reducción incorporada en los motores se obtiene una buena relación velocidad/torque sin las complicaciones mecánicas que conlleva utilizar un sistema de engranajes por separado. 
Por otra parte, los encoders dotan al robot de una resolución de $29\times 64 = 1856$ cuentas por revolución de rueda.\\

Los motores requieren una tensión de $12V$ y pueden llegar a consumir una corriente de $3A$ cuando están bloqueados, por esta razón se incorporó una fuente DC/DC de $12V$ $6A$ \emph{PTN78020} marca \emph{Texas Instruments}.

Para controlar los motores, se realizaron funciones que usan en conjunto las cuentas obtenidas de los encoders, variaciones de la señal PWM y combinaciones en los pines de los puentes H. Estas funciones se encuentran en el archivo \emph{motores.c}.

\subsection{Funcionamiento de los encoders}

Ambos motores tienen un sensor de efecto Hall en el extremo; estos, gracias a la circuiteria integrada, emiten un pulso cuadrado por cada vez que se da el efecto Hall en ellos. \\
Sabiendo que por cada revolución de motor se producen 64 pulsos cuadrados y teniendo en cuenta la reducción de 29:1 en los motores, se puede determinar con precisión la cantidad de giro en cada rueda.
Para que el microcontrolador pueda llevar la cuenta de los encoders, se conecto la salida de cada uno a los pines de interrupción externa INT1 e INT2. De esta forma; cada vez que hay un flanco ascendente en alguna de las interrupciones, se incrementa una variable.
...
En la hoja de datos del \emph{ATMEGA16} se especifica que la máxima tensión admisible para un cero lógico (con el pin configurado como entrada), es de $0.2 \times VCC$ , en nuestro caso $VCC= 5 V$ $\Rightarrow$ $V_{IL_{max}}=1 V$. Por lo tanto una tensión que no supere 1 Volt, será considerada como un cero lógico.
...
El presente proyecto se puede conectar a un PC, por medio de un puerto RS232.
Esta función es utilizara para \emph{debugging}.

Se configuró la \emph{UART} para enviar y recibir datos, y se configuró una interrupción, que se activa cuando se recibe un dato por el puerto serie.
...
Para el diseño del robot, primero se analizó la forma en la cual éste se desplazaría dentro del laberinto. Como, entre los moviemientos a realizar, hay que rotar 90° o 180° se pensó que la estructura del robot debía ser circular, para evitar complicaciones al momento de rotar. \\
...
Durante la realización del proyecto se hallaron varios problemas. Se lista a continución algunos de ellos:

\begin{itemize}
\item Al momento de comprar los motores, no se analizó como encastrar las ruedas. Por lo que se tuvo que mandar a fabricar un adaptador especial.
\item Las ruedas compradas eran muy pequeñas. Por lo que se adaptaron rodillos de una impresora, para cumplir la función de rueda.
\item Se tuvo que crear un adaptador especial para sostener los motores.
\item Durante toda la experiencia, hubo muchos problemas con las placas. Desde pistas cortadas, agujeros mal colocados o errores en el diametro de los mismos, errores en las dimensiones de los componentes. Por ejemplo, la placa de los puente H, se fabricó 3 veces, ya que se hallaron problemas, tanto de diseño como de fabricación.
\end{itemize}
\vspace{5mm}
...
También, el proyecto a sido galardonado con el segundo premio en la categoría \emph{Laberinto}, en la competencia de robótica \emph{Megaman 2013} organizada por el \emph{Club de Robótica} de la \emph{Facultad de ingeniería de la Universidad de Buenos Aires}.\\

De todas formas, hay ciertos aspectos en los cuales, se realizaran algunas mejoras, como por ejemplo:
\begin{itemize}
 
\item Mejorar el sistema de interrupciones: Como se utilizó una OR de diodos, para conectar varios sensores a la misma interrupción, a veces, el funcionamiento de un sensor, afecta el funcionamiento de otro sensor.
\item Mejorar la estructura del robot: Se fabricará un disco de acrílico o fibra para montar ahí los motores, y luego atornillas la placa de alimentación sensores. De esta forma, se dará mayor estabilidad y mejor tracción.
\item Mejorar el control de las ruedas: Mejorar la forma en que se controla cuanto rota cada rueda. 
\item Mejorar los sensores: Si bien los resultados obtenidos han sido satisfactorios, se propone mejorar los sensores del suelo y estudiar la posiblidad de utilizar otros sensores para medir distancia a las paredes. 
\item Implementar algoritmos de autoaprendizaje. Optimizar la forma en que el robot recorre el laberinto.
\end{itemize}
...
\item Dia (v0.97.1) Creación de diagramas de flujo y bloques. (S.O. Linux - Debian)
\item Kile (v2.0.85) Entorno gráfico para desarrollar código \LaTeX. (S.O. Linux - Debian)
\item Mercurial (v1.6.4) Sistema para el control de versiones.
\item TortoiseHG (v1.1.1) Entorno gráfico para utilización de Mercurial. (S.O. Linux - Debian)
