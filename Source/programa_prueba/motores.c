...
unsigned long cuenta_motor_derecha, cuenta_motor_izquierda;
...
/*
 ...
 void avanzar_celda()
 {
    ...
    Cambio 3/6/13 20hs : luego de cada comparacion , el PWM corregido es solo de la rueda izquierda, esto es para evitar
    que el robot aumente su velocidad "indefinidamente".
    ...
    if((cuenta_motor_izquierda || cuenta_motor_derecha)>=(fraccion))    //cuando alguno de los dos motores llegue a cuenta/fraccion de lo que se debe mover para recorrer una celda
        ...
        if(cuenta_encoder_izquierda > cuenta_encoder_derecha)   //si la rueda de la izquierda llego antes que la derecha (notese que uso cuentas de encoder)
            ...
            velocidad_izq=velocidad_izq+(diferencia*proporcion);    //aumento velocidad en rueda izquierda
            }
        else if(cuenta_encoder_izquierda < cuenta_encoder_derecha)
            {
            diferencia=cuenta_motor_derecha-cuenta_motor_izquierda;
            velocidad_izq=velocidad_izq-(diferencia*proporcion);    //reduzco la velocidad de la rueda izquierda
            }
